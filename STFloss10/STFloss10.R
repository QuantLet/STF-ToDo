...
# Quantlet:     STFloss10
# ---------------------------------------------------------------------
# Description:  STFloss10 plots the Danish fire data simulation results
#               for a NHPP with log-normal claim sizes and a NHPP with
#               Pareto claim sizes. The lines are the sample 0.001,
#               0.01, 0.05, 0.25, 0.50, 0.75, 0.95, 0.99, 0.999-quantile
#               lines based on 3000 trajectories of the risk process
#               (dfl.txt).
# ---------------------------------------------------------------------
# Usage:        -
# ---------------------------------------------------------------------
# See also:     STFloss01, STFloss02, STFloss03, STFloss04, STFloss05
#               STFloss06, STFloss07, STFloss08, SFTloss09
# ---------------------------------------------------------------------
# Inputs:       none
# ---------------------------------------------------------------------
# Output:       Plots of the Danish fire data simulation results
#               for a NHPP with log-normal claim sizes and a NHPP with
#               Pareto claim sizes. The lines are the sample 0.001,
#               0.01, 0.05, 0.25, 0.50, 0.75, 0.95, 0.99, 0.999-quantile
#               lines based on 3000 trajectories of the risk process
...
##########################################################
################ Subroutine simNHPP ######################
#### simulation of a non-homogeneous poisson process #####
##########################################################
...
######################################
######## Subroutine simNHPPRP ########
#### simulation of a risk process ####
######################################
...
######################################
######## Subroutine mixexprnd ########
#### simulates mixed exponential  ####
########### random numbers ###########
######################################
...
######################################
######## Subroutine Paretornd ########
### simulates random numbers from  ###
...
##################################################
############# Subroutine simNHPPRPRT #############
#### real-life simulation of a risk process ######
##################################################
...
####################################################
############### subroutine quantiles ###############
####### computes risk process quantile lines #######
####################################################
...
#   y=zeros(1,length(perc));
  ...
  while(i<numofpoints){
    ...
    while(j<=R){
      ...
      val[j,i] = data[ind,j]+(vecstep[i]-tt[ind,j])*(data[ind+1,j]-data[ind,j])/(tt[ind+1,j]-tt[ind,j])
      }
      j        = j+1
    }
    i = i+1
...
tq        = QQ$vecstep
valq      = QQ$y
plot(t1,y1,type="l",col="red3",lty=2,lwd=2.5,xlab="Time (years)",ylab="Capital (DKK million)",ylim=c(0,3000))
lines(t2,y2,col="blue3",lwd=2.5)
for (i in 1:nrow(valq)){
lines(tq,valq[i,]/1e6,col="black")
}
...
dev.new()
plot(pt1,py1,type="l",col="red3",lwd=2.5,lty=2,xlab="Time (years)",ylab="Capital (DKK million)",ylim=c(0,3000))
lines(pt2,py2,col="blue3",lwd=2.5)
for (i in 1:nrow(valq)){
lines(ptq,pvalq[i,]/1e6,col="black")
}
