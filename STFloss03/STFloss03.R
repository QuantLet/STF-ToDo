...
#   negative or a>1.
#   
#   The default values for A, BETA1, BETA2 are 0.5, 1, 2.
      if (missing(alpha)==T) alpha=0.5
      if (missing(beta1)==T) beta1=1
      if (missing(beta2)==T) beta2=2
      if (beta1<=0){
...
plot(x,y1,log="y",col="black", type="l", lwd=2, xlab="x", ylab="PDF(x)",frame=T,ylim=c(10e-3,10e-1))
par(new=T)
plot(x, y2, log="y", axes=F,frame=F, col="red3", type="l", lty=3, lwd=2, ylab="", xlab="",ylim=c(10e-3,10e-1))
par(new=T)
plot(x,y3, log="y", axes=F,frame=F, col="blue3", type="l", lty=2, lwd=2, ylab="", xlab="",ylim=c(10e-3,10e-1))
