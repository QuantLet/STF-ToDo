...
#               simNHPPALP, BondZeroCouponHPP, simHPPALP
#----------------------------------------------------------------------------
# Quantlet:     STFcat08
#----------------------------------------------------------------------------
# Description:  STFcat08 plots the difference between zero-coupon CAT bond prices 
#               in the Burr claim amount case for a homogeneous and 
...
#              computes price of the zero-coupon CAT bond for the given 
...
#	  computes price of the zero-coupon CAT bond for the given 
#     claim amount distribution and the non-homogeneous 
#     Poisson process governing the flow of losses.
#     y = BondZeroCoupon(Z,D,T,r,lambda,parlambda,distr,params,Tmax,N)
# ---------------------------------------------------------------------
# Inputs:       
# Parameter   Z
# Definition  scalar, payment at maturity
...
# Definition  scalar, continuously-compounded discount rate 
# Parameter   lambda
# Definition  scalar, intensity function, if lambda=0, a sine function, if lambda=1, a linear function, if lambda=2, a sine square function
# Parameter   parlambda
# Definition  n x 1 vector, parameters of the intensity function lambda (n=2 for lambda=1, n=3 otherwise)
# Parameter   distrib
# Definition  string, claim size distribution
# Parameter   params
# Definition  n x 1 vector, parameters of the claim size distribution
...
params = c(0.4801,3.9495*1e16,2.1524) #Burr
...
wireframe(z~x*y, data=data, screen = list(z = 70, x = -60),
drape = TRUE,colorkey=F,ticktype="detailed",
  scales = list(arrows=FALSE,col="black",distance=1,tick.number=8,cex=.7,
       x = list(labels=round(seq(min(x),max(x),length=11),1)),
       y = list(labels=round(seq(min(y),max(y),length=11),1)),
       z = list(labels=round(seq(min(z),max(z),length=11),1))),
    xlab = list("",rot=30,cex=1.2),
    ylab = list("",rot=-40,cex=1.2),
    zlab = list("",rot=95,cex=1.1),par.settings = par.set)
