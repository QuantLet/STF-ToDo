...
# Quantlet:     STF2tvch04
# ------------------------------------------------------------------------------
# Description:  STF2tvch04 Plots the estimation output of the tvARCH(1) model for 
#				SP500 data including the predicted volatilities and forecasting 
#				errors relative to GARCH; the numerical values of mean absolute
#				errors are also provided. For faster, but less precise computation,
...
#
# ------------------------------------------------------------------------------
# Output:       Plots the estimation output of the tvARCH(1) model for SP500 data
#				
# ------------------------------------------------------------------------------
# Example:      Plots the estimation output of the tvARCH(1) model for SP500 data
#				including the predicted volatilities and forecasting errors
#				relative to GARCH
# ------------------------------------------------------------------------------
# Author:       Pavel Cizek 20110715
# ------------------------------------------------------------------------------

# Plots the estimation output of the tvARCH(1) model for SP500 data including
# the predicted volatilities and forecasting errors relative to GARCH;
# the numerical values of mean absolute errors are also provided.
# For faster, but less precise computation, the bandwidth can be set 
# to a fixed value at the end of the for loop.
...
tvarch <- function(p, X, t0, bn, lou=FALSE)
{
     ...
     if (lou)
          wint <- c((t0-bn):(t0-1), (t0+1):(t0+bn))
     else wint <- ((t0-bn):(t0+bn))

     if (p == 0) return(list(coefs=mean(X[wint]^2), pred=mean(X[wint]^2)))

     matX <- 1; for (s in (1:p)) matX <- cbind(matX, c(double(p),X[(p-s+1):(n-s)]^2))
     mut <- sum( X[wint]^2 ) / bn
     ...
     if (lou)
          pred  <- sum(at0 * matX[t0-1,])
     else pred  <- sum(at0 * matX[n,])
...
lags <- 21
dev.new()
plot(timet[lags:length(time)], running.mean(errs,lags)/running.mean(ggarch,lags), type="l", xlab="Time", ylab="Ratio of L1 errors", main="TvARCH to global GARCH",xaxp=c(2001,2005,4))
...
print("By year:")
for (ye in 1:4)
     print(c(mean(errs[(250*(ye-1)+1):(250*ye)]), mean(ggarch[(250*(ye-1)+1):(250*ye)])))
print("Total:")
