...
# Quantlet:     STFruin17
# ------------------------------------------------------------------------------
# Description:  STFruin17 produces the relative error of the approximations 
#               (with respect to exact method) of ruin probability in infinite 
#               time treated as a function of the initial capital for the 
#               mixture of 2 exponentials distibution claims.
# ------------------------------------------------------------------------------
# Usage:        -
# ------------------------------------------------------------------------------
# Inputs:       None    
# ------------------------------------------------------------------------------
# Output:       The relative error of the approximations (with respect to exact 
#               method) of ruin probability in infinite time treated as a 
#               function of the initial capital for the mixture of 2 
#               exponentials distibution claims.
# ------------------------------------------------------------------------------
# Example:      -
# ------------------------------------------------------------------------------
# Author:       Zografia Anastasiadou 20110907
# ------------------------------------------------------------------------------
...
dparameters1<-list(c(0.0584,0.9416),c(3.5900e-10,7.5088e-09)) # weights (first vector) and exponential parameters (second vector)

# produces the exact ruin probability in infinite time for insurance collective risk model with mixture of 2 exponentials distribution claims
...
mgfsg<-function(x,k,dparameters){
	# x: scalar, argument of the moment generating function
	# k: scalar, integer, 0 =< k <= 3, order of the derivative
	# dparameters: list of scalars, parameters of gamma distribution
	p1<-dparameters[[1]] 
    p2<-dparameters[[2]] 
...
plot(u1,psi1,type="l",col="red",ylim=c(0,0.801),lwd=3,main="",xlab="u  (USD billion)",ylab=expression(psi(u)),cex.axis=1.6,cex.lab=1.6)
dev.new()
plot(u1,err2,type="l",col="blue",ylim=c(-0.301,0.301),lwd=3,main="",xlab="u  (USD billion)",ylab=expression((psi[i](u)-psi(u))/psi(u)),cex.axis=1.6,cex.lab=1.6)
...
dev.new()
plot(u1,err4,type="l",lty=4,col="red",ylim=c(-1.01,1.01),lwd=3,main="",xlab="u  (USD billion)",ylab=expression((psi[i](u)-psi(u))/psi(u)),cex.axis=1.6,cex.lab=1.6)
