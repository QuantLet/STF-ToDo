...
# Quantlet:     STFdmm03
# ---------------------------------------------------------------------
# Description:  STFdmm03 presents statistical poperties of the minimum
#               spanning tree bidirectional minimum length path and 
#               unidirectional length path. The WIG 20 subset is analysed
#               (gwp.csv)
# ---------------------------------------------------------------------
# Usage:        -
# ---------------------------------------------------------------------
# See also:     STFdmm01, STFdmm02
# ---------------------------------------------------------------------
# Inputs:       none
# ---------------------------------------------------------------------
# Output:       statistical poperties of the minimum spanning tree
#               bidirectional minimum length path and unidirectional 
#               length path. The WIG 20 subset is analysed (gwp.csv)
# ---------------------------------------------------------------------
# Example:      -
# ---------------------------------------------------------------------
# Author:       Matlab: Janusz Miskiewicz
#               R: Awdesch Melzer 20121108
# ---------------------------------------------------------------------
...
for (window in 10:(dl_szer-okno-1) ){
	...
	# notice that in R the kurtosis is centered around 3, while in Matlab around 0
...
par(mfrow=c(2,2))
plot(wynik_wind[,1], wynik_wind_mst[,1],type="l", ylim=c(min(wynik_wind_mst[,1],wynik_wind_umlp[,1],wynik_wind_bmlp[,1]),c(max(wynik_wind_mst[,1],wynik_wind_umlp[,1],wynik_wind_bmlp[,1]))+0.1), ylab="mean",xlab="window",col=2,lwd=3)
...
plot(wynik_wind[,1], wynik_wind_mst[,2],type="l", ylim=c(min(wynik_wind_mst[,2],wynik_wind_umlp[,2],wynik_wind_bmlp[,2]),max(wynik_wind_mst[,2],wynik_wind_umlp[,2],wynik_wind_bmlp[,2])), ylab="std",xlab="window",col=2,lwd=3)
lines(wynik_wind[,1], wynik_wind_umlp[,2],lwd=3, col=3)
lines(wynik_wind[,1], wynik_wind_bmlp[,2],lwd=3,col=4)
legend("topright",c('MST','UMLP','BMLP'),lty=c(1,1,1), col=c(2,3,4),lwd=3)
title("WIG 20")

plot(wynik_wind[,1], wynik_wind_mst[,3],type="l", ylim=c(min(wynik_wind_mst[,3],wynik_wind_umlp[,3],wynik_wind_bmlp[,3]),c(max(wynik_wind_mst[,3],wynik_wind_umlp[,3],wynik_wind_bmlp[,3])+0.4)), ylab="skewness",xlab="window",col=2,lwd=3)
lines(wynik_wind[,1], wynik_wind_umlp[,3],lwd=3, col=3)
lines(wynik_wind[,1], wynik_wind_bmlp[,3],lwd=3,col=4)
legend("topright",c('MST','UMLP','BMLP'),lty=c(1,1,1), col=c(2,3,4),lwd=3)
title("WIG 20")

plot(wynik_wind[,1], (wynik_wind_mst[,4]),type="l", ylim=c(min((wynik_wind_mst[,4]),(wynik_wind_umlp[,4]),(wynik_wind_bmlp[,4])),c(max((wynik_wind_mst[,4]),(wynik_wind_umlp[,4]),(wynik_wind_bmlp[,4]))+0.5)), ylab="kurtosis",xlab="window",col=2,lwd=3)
