...
# Quantlet:     STFcat02
# ------------------------------------------------------------------------------
# Description:  STFcat02 produces the plot of an aggregate loss process together 
#               with a real-world trajectory, two sample quantile lines and mean 
#               of the process.
# ------------------------------------------------------------------------------
# Usage:        -
# ------------------------------------------------------------------------------
# Inputs:       None    
# ------------------------------------------------------------------------------
# Output:       Plot of an aggregate loss process together with a real-world 
#               trajectory, two sample quantile lines and mean of the process.
# ------------------------------------------------------------------------------
# Example:      -
# ------------------------------------------------------------------------------
# Author:       Zografia Anastasiadou 20111010
# ------------------------------------------------------------------------------
...
simNHPPALP<-function(lambda,parlambda,params,T,N){
    # lambda: scalar, intensity function, sine function (lambda=0), linear function (lambda=1) or sine square function (lambda=2) 
    # parlambda: n x 1 vector, parameters of the intensity function lambda (n=2 for lambda=1, n=3 otherwise)
    # params: n x 1 vector, parameters of the lognormal distribution	# T: scalar, time horizon
	# N: scalar, number of trajectories	
	 if (N==1){
...
t2<-(0:(100*T1))/100
RP<-exp(params1[1]+params1[2]^2/2)*(parlambda1[1]*t2-parlambda1[2]/2/pi*(cos(2*pi*(t2+parlambda1[3])) - cos(2*pi*parlambda1[3])))
me<-cbind(t2,RP/1e+9)  
...
qq1<-cbind(qq1[,1],qq1[,2]/1e+9)
qq2<-cbind(qq2[,1],qq2[,2]/1e+9)

plot(y1,type="l",col="blue",ylim=c(0,120),xlab="Years",ylab="Aggregate loss process (USD billion)",cex.lab=1.4,cex.axis=1.4,lwd=2)
