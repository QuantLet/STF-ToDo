...
# Quantlet:     STF2tvch06
# ------------------------------------------------------------------------------
# Description:  STF2tvch06 evaluates the mean absolute forecast errors of the AWS 
#				model for SP500 data in years 2001--2005  
#				
#               
# ------------------------------------------------------------------------------
# Usage:        -
...
#
# ------------------------------------------------------------------------------
# Output:       Plots of mean absolute forecast errors in the AWS model for SP500 data
#				
# ------------------------------------------------------------------------------
# Example:      Plots of mean absolute forecast errors in the AWS model for SP500 data
# ------------------------------------------------------------------------------
# Author:       Pavel Cizek 20110715
# ------------------------------------------------------------------------------

# Evaluates the mean absolute forecast errors of the AWS model for 
# SP500 data in years 2001--2005
...
p <- 1; n <- length(dat0)
matX <- NULL; for (s in (1:p)) matX <- cbind(matX, c(double(p),dat0[(p-s+1):(n-s)]^2))
...
lags <- 21
dev.new()
plot(timet[lags:length(time)], running.mean(errs,lags)/running.mean(ggarch,lags), type="l", xlab="Time", ylab="MAPE rel. to GARCH(1,1)",xaxp=c(2001,2005,4))
