...
# Quantlet:     STFfft01
#----------------------------------------------------------------------------
# Description:  STFfft01 plots of implied volatility of DAX options on 
#               January, 4th 1999, using kernel regression employing a quartic
#               kernel with bandwidth=c(0.05, 0.3) for moneyness and time to
#               maturity respectively.
#---------------------------------------------------------------------------- 
# Author:       R: Awdesch Melzer 20121211
#---------------------------------------------------------------------------- 
...
wireframe(IV~MON+MAT,drape=T,ticktype="detailed", pts=pts,main="",zlim=c(0.25,0.6), aspect = c(1,1),scales=list(arrows=FALSE, y=list(labels=round(seq(firstmat,lastmat,length=6),2)),x=list(labels=round(seq(firstmon,lastmon,length=6),2)),z=list(labels=round(seq(0.25,max(IV),length=8),2))), ylab=list("Time to Maturity",rot=-8), xlab=list("Moneyness",rot=25), zlab=list("Implied Volatility",rot=94) , panel.3d.wireframe = function(x,y,z,xlim,ylim,zlim,xlim.scaled, 
