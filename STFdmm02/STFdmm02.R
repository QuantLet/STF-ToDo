...
# Quantlet:     STFdmm02
# ---------------------------------------------------------------------
# Description:  STFdmm02 presents statistical poperties of the minimum 
#               spanning tree, bidirectional minimum length path and 
#               unidirectional length path for company data (close.csv)
#               The S&P 500 subset is analysed.
# ---------------------------------------------------------------------
# Usage:        -
# ---------------------------------------------------------------------
# See also:     STFdmm01
...
# Inputs:       none
# ---------------------------------------------------------------------
# Output:       presents statistical poperties of the minimum 
#               spanning tree, bidirectional minimum length path and 
#               unidirectional length path.
#               The S&P 500 subset is analysed.
# ---------------------------------------------------------------------
# Example:      -
# ---------------------------------------------------------------------
# Author:       Matlab: Janusz Miskiewicz
#               R: Awdesch Melzer 20121107
# ---------------------------------------------------------------------
...
# load library moments for calculation of skewness, kurtosis, etc.
...
par(mfrow=c(2,2))
plot(wynik_wind[,1], wynik_wind_mst[,1],type="l", ylim=c(min(wynik_wind_mst[,1],wynik_wind_umlp[,1],wynik_wind_bmlp[,1]),c(max(wynik_wind_mst[,1],wynik_wind_umlp[,1],wynik_wind_bmlp[,1]))+0.15), ylab="mean",xlab="window",col=2,lwd=3)
...
plot(wynik_wind[,1], wynik_wind_mst[,3],lwd=3,type="l", ylim=c(min(wynik_wind_mst[,3],wynik_wind_umlp[,3],wynik_wind_bmlp[,3]),c(max(wynik_wind_mst[,3],wynik_wind_umlp[,3],wynik_wind_bmlp[,3])+0.7)), ylab="skewness",xlab="window",col=2)
lines(wynik_wind[,1], wynik_wind_umlp[,3],lwd=3, col=3)
lines(wynik_wind[,1], wynik_wind_bmlp[,3],lwd=3,col=4)
legend("topright",c('MST','UMLP','BMLP'),lty=c(1,1,1), col=c(2,3,4),lwd=3)
title("S&P 500")

plot(wynik_wind[,1], (wynik_wind_mst[,4]),lwd=3,type="l", ylim=c(min((wynik_wind_mst[,4]),(wynik_wind_umlp[,4]),(wynik_wind_bmlp[,4])),c(max((wynik_wind_mst[,4]),(wynik_wind_umlp[,4]),(wynik_wind_bmlp[,4]))+0.8)), ylab="kurtosis",xlab="window",col=2)
